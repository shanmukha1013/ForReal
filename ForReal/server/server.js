...
// Load environment variables (equivalent to require('dotenv').config() in ESM)
dotenv.config();
...
// Use full error logging on failure to aid Render debugging.
mongoose.connect(process.env.MONGO_URI, {
  // Mongoose 6+ uses sane defaults; options can be added here if needed
})
  .then(() => {
    console.info('MongoDB Connected');
    const listenPort = process.env.PORT || 10000;
    httpServer.listen(listenPort, () => {
      console.info(`Server running on port ${listenPort}`);
      console.info(`[Server] Environment: ${process.env.NODE_ENV || 'development'}`);
    });
  })
  .catch((err) => {
    console.error('MongoDB Error: Failed to connect to MongoDB');
    console.error(err && err.stack ? err.stack : err);
    // Exit with non-zero code so Render shows the failure
    process.exit(1);
  });
